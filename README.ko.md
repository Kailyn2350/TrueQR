...
1.  **이미지를 그레이스케일로 변환하고 크기 조정:**
    ...
    - 이 단계는 색상 정보를 제거하여 이미지를 단순화하고, 크기를 일관되게 만들어 주요 구조적 특징에만 집중하도록 합니다.

2.  **2D 이산 코사인 변환(DCT) 적용:**
    그레이스케일 이미지 $I$에 **2D DCT(이산 코사인 변환)**를 적용합니다.
    $$
    C = \text{DCT}(I)
    $$
    - DCT는 이미지를 **공간 영역(spatial domain)**에서 **주파수 영역(frequency domain)**으로 변환합니다.
    - 이미지를 **저주파수** 성분(큰 구조)과 **고주파수** 성분(세부 사항)으로 분리합니다.

3.  **저주파수 $8 \times 8$ 영역 추출:**
    DCT 행렬에서 $8 \times 8$ 크기의 저주파수 영역 $C_{\text{low}}$를 추출합니다. 이 부분은 이미지의 **핵심적인 구조적 특징**을 나타냅니다.
    - 이 저주파수 성분들은 일반적으로 이미지의 **전반적인 밝기**와 **형태**를 나타냅니다.

4.  **중간값 계산:**
    추출된 저주파수 영역 $C_{\text{low}}$의 계수들에서 DC 성분 $C[0,0]$을 제외하고 **중간값($m$)**을 계산합니다.
    $$
    m = \text{median}(C_{\text{low}}[1:, 1:])
    $$
    - 이 중간값은 해시를 생성하기 위한 임계값으로 사용되며, 저주파수 성분들의 "평균적인" 값을 나타냅니다.

5.  **63비트 해시 생성:**
    각 비트가 **저주파수 영역**의 계수에 해당하도록 **63비트 해시** $H$를 생성합니다. 비트는 해당 계수가 중간값보다 큰지에 따라 할당됩니다.
    $$
    h_i = 
    \begin{cases} 
    1 & \text{if } c_i > m \\
    ...
    \end{cases}
    $$
    - 이 과정은 63비트의 **이진 문자열**을 생성하며, 이것이 이미지의 **지각 해시**가 됩니다. 이 해시는 이미지의 저주파수 구조를 고유하게 나타냅니다.
...
$$d_H(H_{\text{test}}, H_{\text{ref}}) = \sum_{i=1}^{63} \left| h_{\text{test}, i} - h_{\text{ref}, i} \right|$$- 이 해밍 거리가 특정 임계값 $\tau_p$보다 작거나 같으면 검증을 통과합니다.$$d_H(H_{\text{test}}, H_{\text{ref}}) \leq \tau_p$$
